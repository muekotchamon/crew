...
import type { CrewEntry, CrewPdfSignatures } from "./crewTypes";
...
type Props = {
  ...
  onCrewPdfSigned: (crewId: string, kind: "compensation" | "waiver", dataUrl: string) => void;
  ...
  waiverPreviewOpenRef?: MutableRefObject<((crewId: string) => void) | null>;
};
...
function waiverFilename(crew: CrewEntry): string {
  const slug = (crew.code || crew.id).replace(/[^\w.-]+/g, "-");
  return `Waiver-Release-of-Lien-${slug}.pdf`;
}

function crewInstallTotal(crew: CrewEntry): number {
  return crew.workItems.reduce((s, w) => s + (Number(w.installCost) || 0), 0);
}
...
export default function DocumentsPanel({
  ...
  const makePreviewBlob = useCallback(
    async (
      ...
      if (docKind === "compensation") {
        ...
        const crewTotal = agreementRows.reduce((s, r) => s + r.installationCost, 0);
        const payeeName =
        ...
        return pdf(
          <CompensationAgreementDocument
            ...
            rows={agreementRows}
            totalInstallationCost={crewTotal}
            payeeCompanyName={payeeName}
            signatureDataUrl={signatureDataUrl}
          />
        ).toBlob();
      }
      const waiverCrewId = options?.waiverCrewId;
      if (!waiverCrewId) throw new Error("waiverCrewId is required for waiver PDF");
      const waiverCrew = crews.find((c) => c.id === waiverCrewId);
      if (!waiverCrew) throw new Error("Crew not found for waiver PDF");
      const waiverPay = crewInstallTotal(waiverCrew);
      const subName =
        waiverCrew.employeeName?.trim() || DEFAULT_JOB_META.subcontractorCompany;
      const nm = waiverCrew.employeeName?.trim();
      const printLine = nm
        ? [nm, waiverCrew.serviceType].filter(Boolean).join(" — ")
        : DEFAULT_JOB_META.printNameTitle;
      return pdf(
        <WaiverLienDocument
          logoSrc={logoSrc}
          jobNumber={DEFAULT_JOB_META.jobNumber}
          subcontractorCompany={subName}
          paymentAmount={waiverPay}
          customerName={DEFAULT_JOB_META.customerName}
          customerAddress={DEFAULT_JOB_META.customerAddress}
          printNameTitle={printLine}
          signatureDataUrl={signatureDataUrl}
        />
      ).toBlob();
    },
  ...
  async function applySignatureToPreview(dataUrl: string) {
    ...
    try {
      const blob = await makePreviewBlob(current.docKind, {
        ...
        waiverCrewId: current.docKind === "waiver" ? current.waiverCrewId : undefined,
      });
      const url = URL.createObjectURL(blob);
      setPreview({
        ...current,
      ...
      if (current.docKind === "compensation" && current.compensationCrewId) {
        onCrewPdfSigned(current.compensationCrewId, "compensation", dataUrl);
      } else if (current.docKind === "waiver" && current.waiverCrewId) {
        onCrewPdfSigned(current.waiverCrewId, "waiver", dataUrl);
      }
      setSignModalOpen(false);
  ...
  const openCompensationPreview = useCallback(
    async (crewId: string) => {
      ...
      setGenerating(true);
      try {
        const savedSig = crewPdfSignatures[crewId]?.compensationDataUrl;
    ...
    [crews, crewPdfSignatures, generating, makePreviewBlob]
  );
  ...
  const openWaiverPreview = useCallback(
    async (crewId: string) => {
      if (generating) return;
      const crew = crews.find((c) => c.id === crewId);
      if (!crew) return;
      setGenerating(true);
      try {
        const savedSig = crewPdfSignatures[crewId]?.waiverDataUrl;
        ...
        setPreview({
          url,
          blob,
          filename: waiverFilename(crew),
          title: `Waiver and Release of Lien — ${displayLabel}`,
          docKind: "waiver",
          waiverCrewId: crewId,
          signatureDataUrl: savedSig,
        });
  ...
  useEffect(() => {
    if (!waiverPreviewOpenRef) return;
    waiverPreviewOpenRef.current = (crewId: string) => {
      void openWaiverPreview(crewId);
    };
    return () => {
      waiverPreviewOpenRef.current = null;
    };
  }, [waiverPreviewOpenRef, openWaiverPreview]);
